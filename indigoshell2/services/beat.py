"""Shared audio source — two independent backends, as in v1.
...
What the loop changes is the plumbing, not the pipeline. v1 ran a worker
thread per backend because `stdout.read` blocks, and marshalled every
frame back to the UI with `GLib.idle_add`. Here `await readexactly` does
not block, and `aubio.tempo` costs ~13µs per 512-sample hop (~0.1% of a
core at 44.1kHz), so both backends run directly on the shell's one loop
and a callback fires on the thread that owns the widget tree.
"""
...
# Aubio beat tracker config, verbatim from v1.
SAMPLE_RATE = 44100
AUBIO_WIN = 1024
AUBIO_HOP = 512
...
def load_aubio():
    ...
    there is. v1 could import it because its uv tool env was created
    with `include-system-site-packages = true`; this venv now has that
    set too, but `uv sync` rewrites `pyvenv.cfg` and would take the beat
    pulse silently with it. Hence the fallback: probe the base prefix
    directly rather than let a venv rebuild turn the feature off.
...
class BeatDetector:
    ...
    def add_listener(self, fn: Callable[[], None]) -> None:
        """Subscribe to beats. Named as in v1, where beats were the
        detector's primary product and bands the extra."""
