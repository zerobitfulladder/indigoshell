...
v1 ran these on worker threads and marshalled back with `GLib.idle_add`.
Here everything is a coroutine on the one loop, so there is no thread and
nothing to marshal — a callback fires directly on the loop that owns the
widget tree.

Three shapes, same as v1:
