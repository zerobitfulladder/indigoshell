...
Ported from v1, with Gio/GLib swapped for dbus-fast on the daemon's own
asyncio loop. What this module does (in plain English):
...
3. **Pub/sub** — widgets subscribe via `.subscribe(...)` to be notified
   ...
   no idle_add hop like v1 needed.
...
class TrayBroker:
    ...
    def _on_item_changed(self, msg: Message) -> None:
        ...
        # registered under a well-known name won't match — same
        # limitation v1 had, and rare in practice.
