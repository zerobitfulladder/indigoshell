...
The word is framed by four small corner brackets, as in v1: an L of arms
at each corner rather than a closed border, so the frame reads as a
targeting reticle. (The first port drew a full beveled box instead; that
lost the look.) The slow colour pulse is ported verbatim: a sine over
`cycle_s` seconds lerping between two magentas.

The pulse is the first thing in v2 driven by a frame clock rather than a
per-widget timer — the widget declares `animation_fps` and the host
window schedules it, so N animated widgets still cost one repaint per
frame instead of N.
"""
