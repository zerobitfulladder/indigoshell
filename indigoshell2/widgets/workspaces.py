...
v1 used python-xlib on its own connection and needed a 50ms polling tick
to drain events that python-xlib had buffered during reply round-trips.
That whole workaround is gone: this shares the daemon's one xcffib
connection, whose pump already handles that case.
"""
