...
Ported from v1. Two things change with the loop: the one-shot state
queries (optimus-manager, tuned-adm) ran on a worker thread there because
they block for 100-500ms and would have frozen the GTK main thread — here
they're coroutines, so they just await. And GPU sampling is likewise async
rather than a blocking `check_output` on the UI thread.
"""
...
class HardwarePanel(Widget):
    ...
    # row's bar started at its own x. v1 pinned the key to 5 chars.
    METRIC_KEY_CHARS = 5
