...
keybinding opens — `main2.py menu power` — a titled stack of chips, one
...
the shell never needs to know what it is. This is v1's dialog pipeline
reduced to "a function that returns the next menu", now that everything
runs on one loop and a stage can hand a Python list back instead of
writing a JSON manifest for a subprocess orchestrator to find.
