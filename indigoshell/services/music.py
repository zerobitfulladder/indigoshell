...
Tracks player status via `playerctl --follow`. The subprocess is already
on the loop, so this is just an async line reader — no thread, no poll.
...
class MusicStatus:
    ...
    def _start(self) -> None:
        # Deliberately not `--player NAME`, which this did first. With
        # that filter playerctl says *nothing at all*
