...
the X socket with PutImage — 2.3MB and 40 requests for a full-size
panel. Rendering into a GL framebuffer owned by the window removes both;
`eglSwapBuffers` replaces the upload entirely.

Measured on this machine, same shader and window size:
...
def backend(display=None) -> Backend | None:
    ...
    are depth 32. Set INDIGOSHELL_GPU=0 to force the raster path.
    """
