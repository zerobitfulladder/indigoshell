...
click/scroll/menu dispatch. `TrayMenu` renders an item's DBusMenu as a
window of our own, so the rows are styled like the rest of the shell
rather than by whatever toolkit the app happens to use.
...
class _TrayRow(Widget):
    ...
    TRACKING = 1.0    # letter-spacing on menu labels
    MARK = "//"
    MIN_WIDTH = 160
    LINE_W = 1.2
    ...
    def paint(self, canvas: skia.Canvas) -> None:
        ...
        # Right-aligned against the marker.
...
# ── menu (popup content) ─────────────────────────────────────────────
# The menu's stylesheet, as constants.
...
class TrayMenu(Column):
    """An item's DBusMenu, rendered as rows. Submenus expand inline,
    indented under their parent rather than hung off to the side —
