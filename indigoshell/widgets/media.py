...
The widget owns its whole rect, so the visualiser fills the bar height
instead of being cropped to the label, and it shrinks to a glyph when
nothing is playing without any size negotiation. A glitched frame is
just a list of `(text, colour)` runs.

Controls are `playerctl`: click plays/pauses, scroll changes track.
"""
...
# Beat decay, expressed per 30ms. Applied as `RATE ** (dt / 30)` so the
# fade takes the same wall-clock time whatever the frame rate is.
BEAT_DECAY = 0.82
BEAT_DECAY_MS = 30.0
...
class Media(Widget):
    ...
    # ♫ and has no reason to cost the bar a repaint. The marquee is
    # measured in px/second, so its speed is independent of the rate.
    @property
    ...
    def __init__(self, *, player: str | None = None,
        ...
        # 20pt is ~27px at 96 DPI; the size tokens are already pixels,
        # and XL is that size.
    ...
    def _baseline(self, size: float, bold: bool) -> float:
        ...
        of pixels high, so this centres on ink instead.

        Measuring the ink of the *string* centres each title exactly, but
        moves the baseline by up to 2px between titles, and would bounce
        mid-glitch as letters flip to 0/1 and the descenders vanish.
        `fTop`/`fBottom` is the same idea taken from the font
