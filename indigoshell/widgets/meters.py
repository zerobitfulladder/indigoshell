...
bottom-up) in proportion to a value. BarMeter, StatMeter and
BatteryMeter share that geometry and add their own source, labelling and
animation on top; all of them advance on the window's frame clock rather
than timers of their own.
"""
...
def _ticks(canvas: skia.Canvas, x: float, y: float, w: float, h: float,
    ...
    The geometry is integral on purpose. Dividing the span by a segment
    *count* gives a fractional tick width (70px / 20 ticks with
...
class BarMeter(Widget):
    ...
    def measure(self, avail: Size) -> Size:
        # The requested width, not the available one. Growing to fill
        # would overflow the row inside a panel. Set flex=1 on the
        # instance to opt back into filling.
...
class StatMeter(Widget):
    ...
    animation_fps = 2      # one sample per 500ms
...
class BatteryMeter(Widget):
    ...
    Each state selects an animation mode with its own interval, between
    33 and 150ms. Rather than run — and re-create — a timer per mode, the
    widget rides the window's 30fps frame clock and advances its own
    accumulator by the mode's interval: same motion, one timer for the
    whole window.
    """
    ...
    def animate(self, t: float) -> None:
        ...
        if self._poll_acc >= 5000:      # battery is polled every 5s
            self._poll_acc = 0.0
            self._poll()
