"""Notification toasts — one window per toast.
...
meter appears when the `value` hint is set.

Each toast IS a window rather than a row inside a shared one, so the
spawn/despawn effects run per-notification. `NotificationManager`
...
def _plain_body(body: str) -> str:
    """Spec allows a small markup subset in the body; the tags are
    stripped and the entities resolved."""
    return html.unescape(_TAG_RE.sub("", body))


def _wrap(line: str, size: float, max_w: float) -> list[str]:
    """Greedy word wrap; overlong words break mid-word."""
    out: list[str] = []
